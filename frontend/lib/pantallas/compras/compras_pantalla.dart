...
class ComprasServicio {
  ...
  Future<List<dynamic>> listar() async {
    final res = await http.get(Uri.parse(ApiConfig.compras), headers: await _h);
  ...
  Future<Map<String, dynamic>> actualizar(
    ...
    final res = await http.put(Uri.parse('${ApiConfig.compras}/$id'),
        headers: await _h, body: jsonEncode(datos));
    final data = jsonDecode(res.body);
...
class _ComprasPantallaState extends State<ComprasPantalla> {
  ...
  Future<void> _eliminar(Map<String, dynamic> c) async {
    final ok = await _confirmar('Eliminar compra',
        '⚠️ Se eliminará la compra #${c['id']} y se REVERTIRÁ el stock de los productos.\n\nEsta acción NO se puede deshacer.');
    if (ok && mounted) {
      try {
        await _servicio.eliminar(c['id']);
        if (mounted) {
          Notificacion.exito(context, 'Compra eliminada y stock revertido');
          _cargar();
        }
      } catch (e) {
        if (mounted) {
    ...
    child: Row(children: [
      ...
      Container(
        ...
        child: Row(mainAxisSize: MainAxisSize.min, children: [
          _btnFiltro('Todas',      'todas',     Icons.list_rounded),
          _btnFiltro('Recibidas',  'recibida',  Icons.check_circle_rounded),
          _btnFiltro('Pendientes', 'pendiente', Icons.hourglass_empty_rounded),
          _btnFiltro('Canceladas', 'cancelada', Icons.cancel_rounded),
        ]),
      ),
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: Row(children: [
        ...
        // Proveedor
        Expanded(flex: 3, child: Text(compra['proveedor'] ?? '-',
            style: const TextStyle(fontWeight: FontWeight.w600, fontSize: 13),
            overflow: TextOverflow.ellipsis)),
        // Productos
        Expanded(flex: 2, child: Text('${detalles.length} producto(s)',
            style: const TextStyle(fontSize: 13, color: Color(0xFF475569)))),
        // Total
        Expanded(flex: 2, child: Text(
            'Bs ${(compra['total'] ?? 0).toStringAsFixed(2)}',
            style: const TextStyle(fontWeight: FontWeight.bold,
                fontSize: 13, color: ColoresCarolina.celesteOscuro))),
        // Estado
...
class _DetalleCompra extends StatelessWidget {
  ...
  Widget _tarjetaDetalle(Map<String, dynamic> d) {
    final esPorKg = (d['tipo_precio'] as String? ?? '') == 'por_kg';
    final String infoProducto;
    if (esPorKg) {
      infoProducto = 'Peso total: ${d['peso_total_kg'] ?? '-'} kg'
          '${d['num_cajas'] != null ? ' • ${d['num_cajas']} caja(s)' : ''}';
    } else {
      infoProducto =
          '${d['num_cajas']} caja(s) × ${d['unidades_x_caja']} unid. = ${d['cantidad']} unidades';
    }
    ...
    return Container(
      ...
      child: Row(children: [
        ...
        Text('Bs ${(d['subtotal'] ?? 0).toStringAsFixed(2)}',
            style: const TextStyle(
                fontWeight: FontWeight.bold,
                fontSize: 15,
                color: ColoresCarolina.celesteOscuro)),
...
class _FormularioCompraState extends State<_FormularioCompra> {
  ...
  Future<void> _cargarDatos() async {
    try {
      ...
      setState(() {
        _proveedores   = p.where((x) => x['activo'] == true).toList();
        _productos     = pr.where((x) => x['activo'] == true).toList();
      ...
      if (_esEdicion) {
        ...
        for (final d in detalles) {
          final tipoPrecio = d['tipo_precio'] as String? ?? 'por_unidad';
    ...
    try {
      if (item['tipo_precio'] == 'por_kg') {
        final peso   = double.tryParse(
            (item['peso_total_kg'] as TextEditingController).text) ?? 0;
        final precio = double.tryParse(
            (item['precio_kg'] as TextEditingController).text) ?? 0;
        return peso * precio;
      } else {
        final cajas   = int.tryParse(
            (item['num_cajas'] as TextEditingController).text) ?? 0;
        final unidades = int.tryParse(
            (item['unidades_x_caja'] as TextEditingController).text) ?? 0;
        final precio   = double.tryParse(
            (item['precio_unitario'] as TextEditingController).text) ?? 0;
  ...
  double get _totalCompra =>
      _items.fold(0, (sum, item) => sum + _calcularSubtotal(item));
  ...
  Future<void> _guardar() async {
      ...
      if (tipo == 'por_kg') {
        return {
          ...
          'num_cajas':       int.tryParse(
              (item['num_cajas'] as TextEditingController).text) ?? 1,
          'unidades_x_caja': int.tryParse(
              (item['unidades_x_caja'] as TextEditingController).text) ?? 1,
          'cantidad':        1,
        };
      } else {
        final cajas   = int.tryParse(
    ...
    try {
      if (_esEdicion) {
        await widget.servicio.actualizar(widget.compra!['id'], datos);
      } else {
        ...
        child: Column(children: [
          ...
          Container(
            ...
            child: Row(children: [
              ...
              const SizedBox(width: 10),
              Text(
                  _esEdicion ? 'Editar Compra #${widget.compra!['id']}' : 'Nueva Compra',
                  style: const TextStyle(color: Colors.white,
                      fontSize: 17, fontWeight: FontWeight.bold)),
              const Spacer(),
              IconButton(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        DropdownButtonFormField<String>(
                          ...
                          decoration: _deco('Estado de la compra', Icons.info_outline_rounded),
                          items: const [
                            DropdownMenuItem(value: 'recibida',
                                child: Text('✅ Recibida — stock ingresa al inventario')),
                            DropdownMenuItem(value: 'pendiente',
                                child: Text('⏳ Pendiente — en camino, sin stock aún')),
                            DropdownMenuItem(value: 'cancelada',
                                child: Text('❌ Cancelada — no afecta inventario')),
                          ],
                          onChanged: (v) => setState(() => _estado = v ?? 'recibida'),
                        ),
                        const SizedBox(height: 14),
                        ...
                        ..._items.asMap().entries.map((e) =>
                            _ItemCompra(
                              ...
                              onEliminar: () => _eliminarProducto(e.key),
                              onChanged:  () => setState(() {}),
                            )),
                        ...
                        TextFormField(
                          ...
                          maxLength: 300,
                          decoration: _deco(
                              'Nota (opcional)', Icons.note_outlined),
                        ),
                      ],
                    ),
                  ),
          ...
          Padding(
            ...
            child: Row(children: [
              ...
              Expanded(flex: 2, child: ElevatedButton(
                onPressed: _cargando || _cargandoDatos ? null : _guardar,
                style: ElevatedButton.styleFrom(
                    backgroundColor: ColoresCarolina.celeste,
                    foregroundColor: Colors.white,
                    padding: const EdgeInsets.symmetric(vertical: 14),
  ...
  double get _subtotal {
    try {
      if (widget.item['tipo_precio'] == 'por_kg') {
        final peso   = double.tryParse(
            (widget.item['peso_total_kg'] as TextEditingController).text) ?? 0;
        final precio = double.tryParse(
            (widget.item['precio_kg'] as TextEditingController).text) ?? 0;
        return peso * precio;
      } else {
        final cajas   = int.tryParse(
            (widget.item['num_cajas'] as TextEditingController).text) ?? 0;
        final unid    = int.tryParse(
            (widget.item['unidades_x_caja'] as TextEditingController).text) ?? 0;
        final precio  = double.tryParse(
            (widget.item['precio_unitario'] as TextEditingController).text) ?? 0;
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          // Campos cajas y unidades (ambos tipos)
          Row(children: [
            Expanded(child: _campo(
              ctrl: widget.item['num_cajas'] as TextEditingController,
              label: 'Nº de cajas',
          ...
          if (!esPorKg)
            _campo(
              ...
              hint: 'Ej: 8.50',
              validator: (v) =>
                  (double.tryParse(v ?? '') ?? 0) <= 0 ? 'Requerido' : null,
            )
          else
            Row(children: [
              Expanded(child: _campo(
